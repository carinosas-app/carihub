 ...
 * Genera public/js/carihub-tecnologia-sector-render.js

 * node scripts/build-carihub-tecnologia-sector-render.mjs
...
import {
  ...
  TECNOLOGIA_FIELD_REGISTRY,

  PACK_NEGOCIO_SUBS,

} from './tecnologia-packs-v1.mjs';
...
const outPath = path.join(root, 'public/js/carihub-tecnologia-sector-render.js');

const mapa = JSON.parse(fs.readFileSync(path.join(root, 'scripts/mapa-registro-categorias.json'), 'utf8'));

const catalogRows = mapa.matrix.filter((r) => r.sectorId === 'tecnologia');

const { SUB_DELTAS } = buildTecnologiaSubDeltas(catalogRows);
...
function buildPreviewFicha(delta) {
  ...
  return {
    ...
    stats: ['tarifaDesde', 'modalidadServicioTI', 'anosExperienciaTI', 'tiempoRespuestaSoporte'].filter((f) =>

      fields.includes(f) || f === 'tarifaDesde'

    ).slice(0, 3),

    rows: fields.slice(0, 14),

    faq: ['garantiaServicio', 'colaboracionesComerciales', 'tiempoRespuestaSoporte', 'coberturaGeografica']
...
const FIELD_LABELS = Object.fromEntries(
  ...
  Object.entries(TECNOLOGIA_FIELD_REGISTRY).map(([k, v]) => [k, v.label || k])
...
const FIELD_TYPES = Object.fromEntries(
  ...
  Object.entries(TECNOLOGIA_FIELD_REGISTRY).map(([k, v]) => [k, v.tipo || 'text'])
...
Object.entries(TECNOLOGIA_FIELD_REGISTRY).forEach(([fieldId, meta]) => {
