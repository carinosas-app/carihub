/**
 * Genera public/js/carihub-comercio-sector-render.js
 * node scripts/build-carihub-comercio-sector-render.mjs
 */
import fs from 'fs';
...
import {
  SUB_TO_PACK,
  PACK_LABELS,
  COMERCIO_FIELD_REGISTRY,
  PACK_NEGOCIO_SUBS,
} from './comercio-packs-v1.mjs';
import { buildComercioSubDeltas } from './comercio-sub-deltas-v1.mjs';

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const outPath = path.join(root, 'public/js/carihub-comercio-sector-render.js');
const mapa = JSON.parse(fs.readFileSync(path.join(root, 'scripts/mapa-registro-categorias.json'), 'utf8'));
const catalogRows = mapa.matrix.filter((r) => r.sectorId === 'comercio');
const { SUB_DELTAS } = buildComercioSubDeltas(catalogRows);
...
function buildPreviewFicha(delta) {
  ...
  return {
    ...
    stats: ['tarifaDesde', 'modalidadVentaComercio', 'entregaDomicilio', 'flotaEntrega'].filter((f) =>
      fields.includes(f) || ['tarifaDesde', 'modalidadVentaComercio'].includes(f)
    ).slice(0, 3),
    rows: fields.slice(0, 14),
    faq: ['entregaDomicilio', 'formasPagoComercio', 'coberturaGeografica', 'volumenMinimoPedido', 'colaboracionesComerciales', 'flotaEntrega']
...
const FIELD_LABELS = Object.fromEntries(
  Object.entries(COMERCIO_FIELD_REGISTRY).map(([k, v]) => [k, v.label || k])
);

const FIELD_TYPES = Object.fromEntries(
  Object.entries(COMERCIO_FIELD_REGISTRY).map(([k, v]) => [k, v.tipo || 'text'])
);
...
const ENUM_LABELS = {};
Object.entries(COMERCIO_FIELD_REGISTRY).forEach(([fieldId, meta]) => {
