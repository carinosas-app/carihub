/**
 * Sync schema — deltas detallados sector profesionales (50 subs).
 * Uso: node scripts/apply-profesionales-sub-deltas-schema.mjs
 */
import fs from "fs";
...
Object.assign(ind.meta.fieldRegistry, PROFESIONALES_FIELD_REGISTRY);
Object.assign(ind.plantillasArquetipo, buildPackPlantillas());

const negocioProfIds = new Set(
...
ind.subcategorias = ind.subcategorias.filter(
  (s) => !(s.sectorId === "profesionales" && negocioProfIds.has(s.subcategoriaId))
);

let indPatched = 0;
for (const sub of ind.subcategorias) {
  if (sub.sectorId !== "profesionales") continue;
...
Object.assign(prof.meta?.fieldRegistry || prof.fieldRegistry || {}, PROFESIONALES_FIELD_REGISTRY);
if (prof.plantillasArquetipo) {
  Object.assign(prof.plantillasArquetipo, buildPackPlantillas());
}
let profPatched = 0;
for (const sub of prof.subcategorias) {
  if (sub.sectorId !== "profesionales") continue;
  const d = SUB_DELTAS[sub.subcategoriaId];
  if (!d || d.formularioId !== "profesionista_cedula") continue;
  sub.delta = {
    ...(sub.delta || {}),
...
let negPatched = 0;
const negProfIds = new Set(neg.subcategorias.filter((s) => s.sectorId === "profesionales").map((s) => s.subcategoriaId));
const negTemplate =
  neg.subcategorias.find((s) => s.subcategoriaId === "consultoria-empresarial") ||
  neg.subcategorias.find((s) => s.sectorId === "profesionales");

for (const row of rows) {
  const d = SUB_DELTAS[row.subcategoriaId];
  if (!d || d.formularioId !== "negocio_empresa") continue;
  if (negProfIds.has(row.subcategoriaId)) {
  ...
  neg.subcategorias.push({
    ...
    keywordsIA: [row.subcategoria.toLowerCase(), "profesionales"],
  });
  negProfIds.add(row.subcategoriaId);
  negPatched++;
}
...
for (const row of mapa.matrix) {
  if (row.sectorId !== "profesionales") continue;
  const d = SUB_DELTAS[row.subcategoriaId];
