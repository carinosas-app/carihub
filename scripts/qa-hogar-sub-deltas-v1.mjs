/**
 * QA — MP-HOGAR-DELTAS-V1 blocks + sub-deltas (17 subs).
 * node scripts/qa-hogar-sub-deltas-v1.mjs
 */
import fs from "fs";
import path from "path";
import vm from "vm";
import { fileURLToPath } from "url";
import { SUB_TO_PACK, PACK_IDS } from "./hogar-packs-v1.mjs";
import {
  SUB_EXTRA_PROFILES,
  MODALIDAD_SERVICIO_HOGAR,
  HIDE_ADULT_LEAKS,
} from "./hogar-sub-extra-fields.mjs";
...
const { SUB_DELTAS } = buildHogarSubDeltas(rows);

ok("17 subs hogar mapa", rows.length === 17, String(rows.length));
ok("17 sub-deltas", Object.keys(SUB_DELTAS).length === 17, String(Object.keys(SUB_DELTAS).length));
ok("17 SUB_TO_PACK", Object.keys(SUB_TO_PACK).length === 17);
ok("17 SUB_EXTRA_PROFILES", Object.keys(SUB_EXTRA_PROFILES).length === 17);

const modalidadValues = MODALIDAD_SERVICIO_HOGAR.map((o) => o.value);
ok("modalidad sin hotel", !modalidadValues.includes("hotel"), modalidadValues.join(","));

const deltasPath = path.join(root, "public/js/data/registro-hogar-sub-deltas.js");
if (fs.existsSync(deltasPath)) {
  ...
  ok(
    ...
    Object.keys(ctx.window.CARIHUB_HOGAR_SUB_DELTAS || {}).length === 17
  );
} else {
  ok("runtime sub-deltas file", false, "Ejecuta generate-hogar-sub-deltas.mjs");
}
...
if (fs.existsSync(blocksPath)) {
  ...
  api = ctx.window.CARIHUB_REGISTRO_HOGAR_SECTOR_BLOCKS;
...
for (const subId of Object.keys(SUB_TO_PACK)) {
  ...
  ok(`diferenciador ${subId}`, (extra?.extraFields || []).includes("diferenciadorHogar"));

  const modalOpts = extra?.fieldOptions?.modalidadServicioHogar;
  if (modalOpts) {
  ...
  if (api) {
    ...
    ok(`formulario ${subId}`, cfg.formularioId === "persona_independiente", d.formularioId);
    ...
    ok(`fotosMin ${subId}`, cfg.fotosMin === 2, String(cfg.fotosMin));
    cfg.obligatorios.forEach(function (key) {
      ...
      if (key === "geo") return;
      ok(`obligatorio field ${subId}:${key}`, found || key === "alias", key);
    });
  }
}
