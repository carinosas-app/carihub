/**
 * Genera public/js/carihub-mascotas-sector-render.js
 * node scripts/build-carihub-mascotas-sector-render.mjs
 */
import fs from 'fs';
...
const outPath = path.join(root, 'public/js/carihub-mascotas-sector-render.js');
const mapa = JSON.parse(fs.readFileSync(path.join(root, 'scripts/mapa-registro-categorias.json'), 'utf8'));
const catalogRows = mapa.matrix.filter((r) => r.sectorId === 'mascotas');
const { SUB_DELTAS } = buildMascotasSubDeltas(catalogRows);
...
function buildPreviewFicha(delta) {
  ...
  return {
    ...
    stats: ['tarifaDesde', 'precioConsulta', 'modalidadServicioMascotas', 'tiempoRespuestaMascotas'].slice(0, 3),
    rows: fields.slice(0, 14),
    faq: ['emergenciasMascotas', 'coberturaGeografica', 'colaboracionesComerciales'].filter((f) => fields.includes(f)).slice(0, 3),
  };
}
...
const FIELD_LABELS = Object.fromEntries(
  Object.entries(MASCOTAS_FIELD_REGISTRY).map(([k, v]) => [k, v.label || k])
);

const FIELD_TYPES = Object.fromEntries(
  Object.entries(MASCOTAS_FIELD_REGISTRY).map(([k, v]) => [k, v.tipo || 'text'])
);
...
const ENUM_LABELS = {};
Object.entries(MASCOTAS_FIELD_REGISTRY).forEach(([fieldId, meta]) => {
