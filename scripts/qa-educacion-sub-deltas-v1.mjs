/**
 * QA — MP-EDUCACION-DELTAS-V1 blocks + sub-deltas (30 subs).
 * node scripts/qa-educacion-sub-deltas-v1.mjs
 */
import fs from "fs";
...
import {
  ...
  MODALIDAD_EDUCACION,
  HIDE_ADULT_LEAKS,
} from "./educacion-sub-extra-fields.mjs";
...
const rows = mapa.matrix.filter((r) => r.sectorId === "educacion");
const { SUB_DELTAS, SUB_CANON_META } = buildEducacionSubDeltas(rows);

ok("30 subs educacion mapa", rows.length === 30, String(rows.length));
ok("30 sub-deltas", Object.keys(SUB_DELTAS).length === 30, String(Object.keys(SUB_DELTAS).length));
ok("30 SUB_TO_PACK", Object.keys(SUB_TO_PACK).length === 30);
ok("30 SUB_EXTRA_PROFILES", Object.keys(SUB_EXTRA_PROFILES).length === 30);
ok("5 packs", PACK_IDS.length === 5, PACK_IDS.join(","));

const modalidadValues = MODALIDAD_EDUCACION.map((o) => o.value);
ok("modalidad sin hotel escort", !modalidadValues.includes("hotel"), modalidadValues.join(","));

const deltasPath = path.join(root, "public/js/data/registro-educacion-sub-deltas.js");
if (fs.existsSync(deltasPath)) {
  ...
  ok(
    ...
    Object.keys(ctx.window.CARIHUB_EDUCACION_SUB_DELTAS || {}).length === 30
  );
} else {
  ok("runtime sub-deltas file", false, "Ejecuta generate-educacion-sub-deltas.mjs");
}

const blocksPath = path.join(root, "public/js/data/registro-educacion-blocks.js");
let api = null;
if (fs.existsSync(blocksPath)) {
  ...
  api = ctx.window.CARIHUB_REGISTRO_EDUCACION_SECTOR_BLOCKS;
...
for (const subId of Object.keys(SUB_TO_PACK)) {
  ...
  ok(`diferenciador ${subId}`, (extra?.extraFields || []).includes("diferenciadorEducacion"));
  ...
  const modalOpts = extra?.fieldOptions?.modalidadEducacion;
  if (modalOpts) {
  ...
  if (api) {
    ...
    ok(`buildConfig sector ${subId}`, cfg?.sectorId === "educacion");
    ...
    if (PACK_PROFESIONISTA_SUBS.has(subId)) {
      ...
      ok(`vet block ${subId}`, (cfg?.blocks || []).some((b) => b.id === "packB_prof"));
    } else if (PACK_NEGOCIO_SUBS.has(subId)) {
      ok(`negocio base ${subId}`, (cfg?.blocks || []).some((b) => b.id === "educacionNegocioBase"));
    } else {
      ok(`persona base ${subId}`, (cfg?.blocks || []).some((b) => b.id === "educacionBase"));
    }
  }
}
