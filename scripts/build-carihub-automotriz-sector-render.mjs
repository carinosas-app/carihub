/**
 * Genera public/js/carihub-automotriz-sector-render.js
 * node scripts/build-carihub-automotriz-sector-render.mjs
 */
import fs from 'fs';
...
import {
  SUB_TO_PACK,
  PACK_LABELS,
  AUTOMOTRIZ_FIELD_REGISTRY,
  PACK_NEGOCIO_SUBS,
} from './automotriz-packs-v1.mjs';
import { buildAutomotrizSubDeltas } from './automotriz-sub-deltas-v1.mjs';

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const outPath = path.join(root, 'public/js/carihub-automotriz-sector-render.js');
const mapa = JSON.parse(fs.readFileSync(path.join(root, 'scripts/mapa-registro-categorias.json'), 'utf8'));
const catalogRows = mapa.matrix.filter((r) => r.sectorId === 'automotriz');
const { SUB_DELTAS } = buildAutomotrizSubDeltas(catalogRows);
...
function buildPreviewFicha(delta) {
  ...
  return {
    ...
    stats: ['tarifaDesde', 'modalidadServicioAuto', 'tiempoRespuestaAuto', 'anosExperienciaAuto'].filter((f) =>
      fields.includes(f) || ['tarifaDesde', 'modalidadServicioAuto'].includes(f)
    ).slice(0, 3),
    rows: fields.slice(0, 14),
    faq: ['garantiaServicioAuto', 'coberturaGeografica', 'coberturaCarretera', 'colaboracionesComerciales', 'financiamientoDisponible']
...
const FIELD_LABELS = Object.fromEntries(
  Object.entries(AUTOMOTRIZ_FIELD_REGISTRY).map(([k, v]) => [k, v.label || k])
);

const FIELD_TYPES = Object.fromEntries(
  Object.entries(AUTOMOTRIZ_FIELD_REGISTRY).map(([k, v]) => [k, v.tipo || 'text'])
);
...
const ENUM_LABELS = {};
Object.entries(AUTOMOTRIZ_FIELD_REGISTRY).forEach(([fieldId, meta]) => {
