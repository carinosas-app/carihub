/**
 * Catálogo oficial de estados y municipios de México (INEGI / AGEEML).
 * Fuente: Catálogo Único de Claves de Áreas Geoestadísticas (INEGI),
 *         vía cisnerosnow/json-estados-municipios-mexico.
 * Contenido: 32 estados, 2476 municipios. Sin duplicados, orden alfabético.
 * Nombres de estado alineados al uso de la app (Estado de México, Coahuila, Michoacán, Veracruz).
 *
 * Estructura: MEXICO_ESTADOS_MUNICIPIOS = { "Estado": ["Municipio", ...] }
 *
 * Integración: este archivo se carga DESPUÉS de estados.js y ciudades.js y
 * rellena los catálogos globales existentes para que la cascada país → estado →
 * municipio (ciudad) de México use el catálogo completo, sin cambiar la lógica.
 *   - ESTADOS["México"]  = lista de estados
 *   - CIUDADES["México"] = { estado: [municipios] }
 *
 * NO inventa municipios. Generado por scripts/_gen-mexico-catalogo.mjs.
 */
const MEXICO_ESTADOS_MUNICIPIOS = {
  "Aguascalientes": [
    "Aguascalientes",
    "Asientos",
    "Calvillo",
...
const MEXICO_ESTADOS = Object.keys(MEXICO_ESTADOS_MUNICIPIOS);

function municipiosDeEstadoMexico(estado) {
  if (!estado) return [];
  if (MEXICO_ESTADOS_MUNICIPIOS[estado]) return MEXICO_ESTADOS_MUNICIPIOS[estado].slice();
  const objetivo = String(estado)
    .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    .toLowerCase().trim();
  const clave = MEXICO_ESTADOS.find(
    (e) => e.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim() === objetivo
  );
  return clave ? MEXICO_ESTADOS_MUNICIPIOS[clave].slice() : [];
}

// Integración no destructiva en los catálogos globales (estados.js / ciudades.js).
// Se mutan los objetos existentes (no se reasignan los const) para no romper otros scripts.
(function integrarCatalogoMexico() {
  try {
    if (typeof ESTADOS !== "undefined" && ESTADOS && typeof ESTADOS === "object") {
      ESTADOS["México"] = MEXICO_ESTADOS.slice();
    }
  } catch (e) { /* noop */ }
  try {
    if (typeof CIUDADES !== "undefined" && CIUDADES && typeof CIUDADES === "object") {
      CIUDADES["México"] = MEXICO_ESTADOS_MUNICIPIOS;
    }
  } catch (e) { /* noop */ }
})();

if (typeof window !== "undefined") {
  window.MEXICO_ESTADOS_MUNICIPIOS = MEXICO_ESTADOS_MUNICIPIOS;
  window.MEXICO_ESTADOS = MEXICO_ESTADOS;
  window.municipiosDeEstadoMexico = municipiosDeEstadoMexico;
}
